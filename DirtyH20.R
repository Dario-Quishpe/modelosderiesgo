...
vars <- c("VarDep",  
          ...
          "ANTIG_LABORAL",
          "ANTIGUEDAD_OP_OTROS",
          "MAX_DVEN_SF_3M","numOpsVencidas3M102",
...
h2o.saveModel(object = my_glm, path = "graficas/", force = TRUE)

# Carga de modelo para estimación
my_glm <- h2o.loadModel(path = "graficas/RF")
#CLEAN
info_em <- as.h2o(x = setDT(info_clean)[, vars, with=FALSE])
info_em[,y_em] <- as.factor(info_em[,y_em])
info_clean[, SCORE_RF_dirty := ceiling(1000*as.data.frame(h2o.predict(my_glm, newdata = info_em))[,2])]
summary(info_clean$SCORE_RF_dirty)
#INFO
info_em <- as.h2o(x = setDT(info)[, vars, with=FALSE])
info_em[,y_em] <- as.factor(info_em[,y_em])
info[, SCORE_RF_dirty := ceiling(1000*as.data.frame(h2o.predict(my_glm, newdata = info_em))[,2])]
summary(info$SCORE_RF_dirty)

#DIRTY
info_em <- as.h2o(x = setDT(info_dirty)[, vars, with=FALSE])
info_em[,y_em] <- as.factor(info_em[,y_em])
info_dirty[, SCORE_RF_dirty := ceiling(1000*as.data.frame(h2o.predict(my_glm, newdata = info_em))[,2])]
summary(info_dirty$SCORE_RF_dirty)
...
mod1<-info_clean[ModVal == 0 & VarDep %in% c(0,1,2,3,4)]
val1<-info_clean[ModVal == 1 & VarDep %in% c(0,1,2,3,4)]
...
mod_em1 <- as.h2o(x = setDT(mod1)[, vars, with=FALSE])
val_em1 <- as.h2o(x = setDT(val1)[, vars, with=FALSE])
mod_em1[,y_em] <- as.factor(mod_em1[,y_em])
val_em1[,y_em] <- as.factor(val_em1[,y_em])
...
res_fun <- function(valida, resultado){
  res <- data.frame(VarDep=valida$VarDep,
                    ...
                    SCORE_RF_clean=valida$SCORE_RF_clean)
  return(res)
}
...
mod_glm1 <- setDT(res_fun(mod1, h2o.predict(my_glm, newdata = mod_em1)))
mod_glm1[,list(Min=min(Score), Max=max(Score)),by=Rango][order(Rango)]
mod_glm1[,table(Rango, VarDep)]
val_glm1 <- setDT(res_fun(val1, h2o.predict(my_glm, newdata = val_em1)))
val_glm1[,list(Min=min(Score), Max=max(Score)),by=Rango][order(Rango)]
val_glm1[,table(Rango, VarDep)]
